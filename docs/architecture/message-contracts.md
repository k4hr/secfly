...
Этот документ задаёт архитектурные контракты. На ЭТАПЕ 1 реализованы фундаментальные TypeScript-типы и Zod-проверка универсальной оболочки; предметные события и JSON Schema полного протокола остаются для последующих этапов.
